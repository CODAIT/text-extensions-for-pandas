...
def DataFrameDocumentContainerComponent(widget, dataframe):
    ...
    if len(span_columns) > 0:
        ...
        controls = DataFrameDocumentControlsComponent(widget=widget, span_columns=span_columns)
...
def DataFrameDocumentControlsComponent(widget, span_columns):
    ...
    if len(span_columns) == 1:
        ...
        tag_dropdown = ipw.Dropdown(options=[('None', None), *tag_values], description='tag', value=widget._tag_display)
        ...
        color_dropdown = ipw.Dropdown(options=color_mode_values, description="color_mode", value=widget._color_mode)
    ...
    # If there are multiple span columns, add some additional options
        ...
        # Tag Dropdown
        tag_values = map(lambda v: (v, ("column_data", v)), widget._df.columns.values)
        tag_dropdown = ipw.Dropdown(options=[('None', None), ("Col Headers", ("column_header")), *tag_values], description='tag', value=widget._tag_display)
        tag_dropdown.observe(widget._update_tag, names='value')
        control_widgets.append(tag_dropdown)
...
def _get_span_color(color_mode: str, stack_data, tag: str, color_map: dict, color_index: int) -> tuple:
    """Gets the appropriate color for a span and returns (the color, the updated color map, the updated color index)"""
    if color_mode == 'ROW':
        return (f"#{_COLOR_PALETTE[stack_data[1] % len(_COLOR_PALETTE)]}", color_map, color_index)
    elif color_mode == 'TAG':
        if tag in color_map:
            return color_map[tag], color_map, color_index
        else:
            color_map.update({tag: f"#{_COLOR_PALETTE[color_index % len(_COLOR_PALETTE)]}"})
            color_index += 1
            return color_map[tag], color_map, color_index