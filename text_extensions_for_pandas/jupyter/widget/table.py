...
def DataFrameTableComponent(widget) -> ipw.Widget:
    ...
    def SpanOnBeginChange(change, span, text, column_name, index):
        new_span = Span(widget._df.get(column_name)[index].target_text, change['new'], span.end)
    ...
    def SpanOnEndChange(change, span, text, column_name, index):
        new_span = Span(widget._df.get(column_name)[index].target_text, span.begin, change['new'])
        widget._df.get(column_name)[index] = new_span
        text.value = str(new_span)
        widget._update_document()
    ...
    for index in dataframe.index:
        ...
        select_box.observe(lambda change, index=index: MetaDataOnChange(change, index), names='value')
        cell = ipw.HBox([ipw.HTML(str(index)), select_box], layout=ipw.Layout(justify_content="flex-end", margin='0'))
    ...
    for column in dataframe.columns:
        ...
        table_columns.append(DataFrameTableColumnComponent(widget, is_interactive, dataframe[column], SpanOnBeginChange, SpanOnEndChange, TokenSpanOnBeginChange, TokenSpanOnEndChange))
...
def DataFrameTableColumnComponent(widget, is_interactive, column, SpanOnBeginChange, SpanOnEndChange, TokenSpanOnBeginChange, TokenSpanOnEndChange):
    ...
    if(is_interactive):
        ...
        for column_index in range(len(column)):
            ...
            cell_widget = ipw.HBox(children=[widget_ui], layout=ipw.Layout(justify_content="flex-end", margin='0'))
        ...
        for column_index in range(len(column)):
            ...
            cell_widget = ipw.HBox(children=[ipw.HTML(f"<div>{str(item)}</div>")], layout=ipw.Layout(justify_content="flex-end", margin='0'))
            cell_widget.add_class(f"tep--dfwidget--row-{column_index}")
            column_items.append(cell_widget)
